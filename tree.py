...
class tree(BaseEstimator):
    ...
    def model( self, dataframe, dataframe2, y ):
        elem = self.setup( dataframe, dataframe2, y )
        variables = self.variables( Num_points = elem[0])
        self.constraints( variables, dataframe, points = elem[1], err = elem[2], Y = elem[3])
        self.objective( variables )

    def setup(self, dataframe, dataframe2, y):
        ...
        return (Num_points, points, err, Y)


    def variables(self, Num_points):
        a = []
        for t in self.Tb:
            a.append(self.mdl.binary_var_list(len(self.features), ub=1, lb=0, name='a%d' % (t)))  # 'feature_in_node%d'%(t)

        # for each branch node associate a variable
        b = self.mdl.continuous_var_list(self.Tb, lb=0, ub = 1, name='b')  # 'hyperplane_coefficient'
        #b = self.mdl.integer_var_list(self.Tb, lb=0, ub = 10, name='b')

        # per ogni nodo, indica se si applica lo split
        d = self.mdl.binary_var_list(self.Tb, name='d')  # node_with_split

        # per ogni nodo, è 1 se il punto si trova in quel nodo
        z = self.mdl.binary_var_matrix(Num_points, self.Tl, name='z')  # 'in_leaf_%d_pass_point_%d'

        l = self.mdl.binary_var_list(self.Tl, name='l')  # leaf_with_points

        c = self.mdl.binary_var_matrix(len(self.classes), self.Tl, name='c')  # class_of_leaf_%d_is_%d

        L = self.mdl.continuous_var_list(self.Tl, lb=0, name='L')  # loss_in_leaf

        Nt = self.mdl.continuous_var_list(self.Tl, name='Nt')  # points_in_leaf

        Nkt = self.mdl.continuous_var_matrix(len(self.classes), self.Tl, name='Nkt')  # points_in_leaf_%d_of_class_%d

        return (a, b, d, z, l, c, L, Nt, Nkt)

    def constraints(self, variables, dataframe, points, err, Y):
        ...
        for le in range(len(self.Tl)):
            for k in range(len(self.classes)):
                self.mdl.add_constraint(L[le] >= Nt[le] - Nkt[k, le + self.floorTb] - self.M * (1 - c[k, le + self.floorTb]))

        for le in range(len(self.Tl)):
            for k in range(len(self.classes)):
                self.mdl.add_constraint(L[le] <= Nt[le] - Nkt[k, le + self.floorTb] + self.M * c[k, le + self.floorTb])

        for le in range(len(self.Tl)):
            for k in range(len(self.classes)):
                self.mdl.add_constraint(
                    Nkt[k, le + self.floorTb] == 0.5 * self.mdl.sum((1 + Y[i, k]) * z[i, le + self.floorTb] for i in points))

        for le in range(len(self.Tl)):
            self.mdl.add_constraint(Nt[le] == self.mdl.sum(z[p, le + self.floorTb] for p in points))

        for le in range(len(self.Tl)):
            self.mdl.add_constraint(l[le] == self.mdl.sum(c[k, le + self.floorTb] for k in range(len(self.classes))))


        # addictional constraint Giorgio
        for  k in range(len(self.classes)):
            self.mdl.add_constraint(1 <= self.mdl.sum(c[k, le + self.floorTb] for  le in range(len(self.Tl)) ))
        #end
        ...
        for p in points:
            for le in range(len(self.Tl)):
                for n in self.Ar[le + self.floorTb]:
                    self.mdl.add_constraint(np.dot(dataframe.loc[p], a[n]) >= b[n] - self.M2 * (1 - z[p, le + self.floorTb]))

        for p in points:
            for le in range(len(self.Tl)):
                for m in self.Al[le + self.floorTb]:
                    self.mdl.add_constraint(
                        np.dot(dataframe.loc[p] + self.eps - err * np.ones(len(self.features)), a[m]) + err <= b[
                            m] + self.M1 * (1 - z[p, le + self.floorTb]))

        for p in points:
            self.mdl.add_constraint(self.mdl.sum(z[p, le + self.floorTb] for le in range(len(self.Tl))) == 1)  #

        for le in range(len(self.Tl)):
            for p in points:
                self.mdl.add_constraint(z[p, le + self.floorTb] <= l[le])

        for le in range(len(self.Tl)):
            self.mdl.add_constraint(l[le] * self.Nmin <= self.mdl.sum(z[p, le + self.floorTb] for p in points))

        for t in self.Tb:
            self.mdl.add_constraint(d[t] == self.mdl.sum(a[t][f] for f in self.features))

        for t in self.Tb:
            self.mdl.add_constraint(b[t] <= d[t])
        for t in np.delete(self.Tb, 0):
            self.mdl.add_constraint(d[t] <= d[self.parent[t]])

        for i in range(0, len(self.Tl), 2):
            self.mdl.add_constraint(l[i] <= d[self.find_pt()[i + self.depth]])
    ...
    def objective(self, variables):
        ...
        self.mdl.minimize(self.mdl.sum(L[le] * randL[le] for le in range(len(self.Tl))) + self.alpha * self.mdl.sum(
            d[t] * randa[t] for t in self.Tb) )
        self.mdl.print_information()

    def setCplexSettings(self):
        #self.mdl.parameters.preprocessing.symmetry = 5
        self.mdl.parameters.barrier.convergetol = 1e-2
        #self.mdl.parameters.mip.cuts.cliques = 3
        #self.mdl.parameters.mip.cuts.covers = 3
        self.mdl.parameters.benders.strategy = -1
        #self.mdl.set_time_limit(600)
        self.mdl.parameters.mip.tolerances.mipgap = 0.05
        #self.mdl.parameters.preprocessing.presolve = 0


    def solve(self):

        start = time()
        self.setCplexSettings()
    ...
    def fit(self, dataframe, dataframe2, y):
        ...
        self.model(dataframe, dataframe2, y)
        stats = self.solve()
        sol = self.mdl
    ...
    def test_model(self, dataframe, dataframe2, y):
        ...
        for p in points:
            for le in range(len(self.Tl)):
                for m in self.Al[le + self.floorTb]:
                    mdl.add_constraint(np.dot(dataframe.loc[p] + self.eps_test - err_test * np.ones(len(self.features)),
                                              self.A[m]) + err_test <= self.B[m] + self.M1_test * (
